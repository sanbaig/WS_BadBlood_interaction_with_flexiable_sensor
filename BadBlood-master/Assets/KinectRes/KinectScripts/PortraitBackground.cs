...
public class PortraitBackground : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if(kinectManager && kinectManager.IsInitialized())
		{
            ...
            if (guiTexture != null)
            {
                RectTransform rectTransform = guiTexture.GetComponent<RectTransform>();
                if (rectTransform != null)
                {
                    rectTransform.offsetMin = new Vector2(dOffsetX, rectTransform.offsetMin.y); // 设置左侧偏移
                    rectTransform.sizeDelta = new Vector2(fDeltaWidth, rectTransform.sizeDelta.y); // 设置宽度
                }
            }
